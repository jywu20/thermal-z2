...
struct IsingFieldDPI{L <: AbstractLattice{Int}, V} <: AbstractDiscretePathIntegralConfiguration
    n_τ::Int
    lattice::L
    data::Array{Int, 2}
end
...
timeslice(s::IsingFieldDPI, τ::Int) = s.data[:, τ]

setindex!(s::IsingFieldDPI, v, i::Int, τ::Int) = s.data[i, τ] = v
getindex(s::IsingFieldDPI, i::Int, τ::Int) = s.data[i, τ]
...
function magnetization(s::IsingFieldDPI, τ::Int)
    sum(s.data[:, τ]) / site_number(s.lattice)
end

function Δ_spacial_correlation_term(s::IsingFieldDPI, i::Int, τ::Int)
    lattice = field_lattice(s)
    nearest_site_values = map(j -> s[j, τ], nearest_neighbors(lattice, i))
    - 2 * s[i, τ] * prod(nearest_site_values)
end

function Δ_temporal_correlation_term(s::IsingFieldDPI, i::Int, τ::Int)
...
function accept_rate(model::TransverseFieldIsingModelDPIMetropolisMC, s::IsingFieldDPI, i::Int, τ::Int)
    J_xy = model.J_xy
    J_τ = model.J_τ
